...
def test_ticket_number_auto_generated_when_not_given():
    ticket = Ticket(user_name="Kelvin Muita", user_email="kelvin@example.com", description="Issue")
...
def test_two_tickets_get_different_numbers():
    ticket_a = Ticket(user_name="Kelvin Muita", user_email="kelvin@example.com", description="A")
    ticket_b = Ticket(user_name="Victor Kingara", user_email="victor@example.com", description="B")
...
def test_to_dict_and_from_dict_roundtrip():
    original = Ticket(
        ...
        user_name="Kelvin Muita",
        user_email="kelvin@example.com",
        description="My internet is down",
        assigned_employee="care1@example.com",
        status="pending",
        action_taken="Checked router",
        resolution_summary="",
        user_agreed_to_close=None,
        user_feedback="",
        history=[{"by": "care1@example.com", "action": "Checked router", "status": "pending", "timestamp": "t1"}],
    )

    rebuilt = Ticket.from_dict(original.to_dict())

    assert rebuilt.ticket_number == "TCK-ABCD1234"
    assert rebuilt.description == "I cannot access my Canvas account"
    assert rebuilt.assigned_employee == "care1@example.com"
    assert rebuilt.status == "pending"
    assert rebuilt.history == original.history


def test_from_dict_defaults_missing_optional_fields():
    rebuilt = Ticket.from_dict({"ticket_number": "TCK-XYZ", "user_email": "kelvin@example.com"})
